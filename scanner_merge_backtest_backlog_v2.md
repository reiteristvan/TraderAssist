...
## EPIC E13 — Target Reliability (data-driven profit-taking)

> **Motivation (István, 2026-06-11):** backtest and prior live experience both show the suggested target is too optimistic — fixed-ish ~10% targets fail far more often than a 2:1 R:R can support, and on the old scanner the target was routinely overridden by hand. The current `compute_targets` confluence engine (ported in E4.1) and the fixed/percentage flavor of targeting are the suspects. This epic does NOT guess a new rule — it instruments the backtest to find the expectancy-maximizing target empirically, then bakes in the conservative defaults the data supports. Sequence matters: E13.1 (measure) gates E13.3 (the data-driven rule). E13.2 is an independent quick win that can ship immediately.
>
> **Prerequisite reading for the implementer:** swing_scanner `compute_targets` (line 459), `count_resistance_obstacles` (line 710), `TargetMethod`/`TargetAnalysis` (lines 243–257), and how E4.1 currently picks `suggested_target` and computes `risk_reward`.
>
> **UPDATE (István, 2026-06-11, after running E13.1):** the finding overturned the premise. The majority of failed trades are STOP-OUTS, not time-stop exits — price is reversing after entry, not stalling below a too-far target. Optimistic targets are therefore NOT the live problem (worth watching, not fixing now). Consequences: **E13.2 demoted from "quick win" to low-priority-but-still-worth-shipping** (it only pulls targets in, downside-protected, matches prior manual practice — keep, deprioritize). **E13.3 PARKED** — target distance is not where expectancy leaks; do not calibrate an ATR target rule until/unless the picture changes. The active investigation moves to **E14** (stop-vs-selection diagnosis), which the E13.1 result directly motivated.

### E13.1 — Instrument the backtest with target-distance & exit-reason attribution
**Description:** Make "how far should the target be" a measurable dimension instead of a hunch. Two additions, both in the report layer (E8), no trading-logic change:

(a) **Exit-reason breakdown of failures.** In E8.1's exit-reason table, split non-winning outcomes into `stop_out` vs `time_stop` explicitly (the simulator already records `exit_reason` — just surface the split prominently). Interpretation line: time-stop-heavy failures ⇒ targets too far for the 10-session horizon; stop-out-heavy ⇒ setup breaking, a different problem. This directly answers the question from the discussion: are the "failures" price reversing, or price simply not reaching a too-far target in time?

(b) **Expectancy by target distance.** New report section bucketing trades by the target's distance from entry, expressed BOTH as R-multiple-to-target (target distance ÷ stop distance: 1.0–1.5, 1.5–2.0, 2.0–2.5, 2.5–3.0, 3.0+) and as ATR-multiple (target distance ÷ entry ATR). Per bucket: n, hit rate (% reaching target before stop/time-stop), avg R realized, expectancy. The expectancy column will typically peak at a moderate distance and fall off as hit rate collapses faster than reward grows — that peak is the empirical target rule feeding E13.3.

To compute (b), the simulator must record, per trade, the **target distance at signal time** in both units. Add `target_r: float` (target distance ÷ stop distance) and `target_atr: float` (target distance ÷ atr) to the `Trade` dataclass (E7.1) and to the persisted signal row. Pure addition; no exit-logic change.

**Acceptance criteria:**
1. Report shows stop-out vs time-stop split of all non-winning trades; counts sum to total non-winners.
2. Target-distance section renders both bucketings with hit rate + expectancy per bucket; a hand-built fixture (trades engineered at known distances) produces hand-verified bucket assignment and expectancy.
3. `target_r` / `target_atr` persisted and queryable (round-trips through the DB, E9).
4. No change to any trade's exit outcome vs pre-task (golden simulator fixtures from E7.1 unchanged).

**Estimate:** M. **Dependencies:** E8.1, E7.1, E9.4.

### E13.2 — Conservative-target rule + obstacle rejection [changes trading logic]
**Description:** The independent quick win — matches what István already did by hand, no backtest needed to justify it (it only ever pulls targets IN, never out, so it can't make R:R worse on a per-trade basis, only the qualification cut tighter). Three changes in `scanner/targets.py` / `attach_risk` (E4.1):

(a) **Default to the conservative confluence target.** Where `compute_targets` yields multiple candidates, `suggested_target` becomes the NEAREST meaningful level (first prior swing high / nearest measured-move floor / nearest Fib), not an optimistic blend or far level. Expose the full ladder as `target_candidates` on the result for the UI, but the headline target and the `risk_reward` it implies use the conservative one.

(b) **Obstacle-aware rejection/pull-in.** Using `count_resistance_obstacles(df, entry, target)`: if ≥1 resistance level sits between entry and the chosen target, pull the target IN to just below the first obstacle. If pulling in drops `risk_reward` below a floor constant `MIN_RR = 1.5` (core.py), mark the setup `target_unreliable=True` rather than emitting a fantasy level — these surface but are visually flagged and excluded from the journal's "actionable" default filter.

(c) **R:R floor as a soft gate.** Setups whose conservative target can't clear `MIN_RR` are downgraded (not hard-rejected — logged like a near-miss with reason `"rr_below_floor"`), so E8.3 attribution can later confirm whether this floor helps.

This is a trading-logic change ⇒ golden masters (E0.3) for any fixture whose target moves must be updated WITH justification, and the change is called out in the task summary.

**Acceptance criteria:**
1. Synthetic frame with a swing high 4% above entry and a far Fib 11% above ⇒ `suggested_target` picks the 4% level; `risk_reward` recomputed against it.
2. Frame with resistance between entry and target ⇒ target pulled below it; if resulting RR < 1.5 ⇒ `target_unreliable=True`.
3. `target_candidates` ladder present on the result for the UI.
4. Affected golden masters updated with written justification; unaffected ones unchanged.

**Estimate:** M. **Dependencies:** E4.1.

### E13.3 — ATR-relative target default, calibrated from E13.1 [changes trading logic]
**Description:** Replace any fixed-percentage component of the target with an ATR-relative one, with the multiple CHOSEN from E13.1's expectancy-by-distance result (not picked a priori). Process, in order:
1. Run E13.1's attribution on a backtest over a liquid universe (e.g. S&P 600, 2+ years) for each strategy.
2. Read the expectancy-maximizing ATR-distance bucket per strategy; record the chosen multiple and the supporting numbers in the task summary (this is the audit trail — the constant must be traceable to data).
3. Set per-strategy constants `PULLBACK_TARGET_ATR_MULT` / `BREAKOUT_TARGET_ATR_MULT` in core.py. The conservative-confluence target (E13.2a) and the ATR target combine as `min(conservative_confluence, entry + mult·ATR)` — i.e. take whichever is nearer, so resistance still caps an over-far ATR target and a tight-range stock still gets a reachable target.
4. Re-run the backtest and confirm expectancy improved (or document why the change is kept despite flat expectancy — e.g. fewer fantasy targets, better live tradability).

Because the constant is data-derived, guard against overfitting: the chosen multiple must also hold up on the E-walk-forward held-out period (see note below) — if it only works on the tuning window, it's an artifact, revert.

**Acceptance criteria:**
1. Task summary contains the E13.1 table the multiple was read from, per strategy.
2. Combined target = nearer of conservative-confluence and ATR-target (unit-tested both ways: ATR nearer, confluence nearer).
3. Backtest expectancy before/after reported; if not improved, explicit justification for keeping the change.
4. Multiple validated on out-of-sample data, not only the tuning window.

**Estimate:** M. **Dependencies:** E13.1, E13.2.

### E13.4 — OPTIONAL: partial exit / trailing stop (phase two)
**Description:** Only after E13.2/E13.3 land and a single-target rule is solid. Scale out: take half at the conservative first target (locks the high-probability portion), trail the remainder with an ATR/chandelier stop to capture runners. This stops asking the target to be a prediction. Cost: the simulator (E7) must support two exit events per trade (a `partial_exit` record + a final exit), metrics must handle blended R, and it complicates the IBKR bracket-order workflow — so it's explicitly phase two and gated on the single-target version being proven first. Backtest it head-to-head against the single-target rule before adopting.

**Acceptance criteria:**
1. Simulator supports a two-leg exit with blended R, behind a flag (single-target path unchanged when off).
2. Head-to-head backtest: partial-exit expectancy vs single-target expectancy reported on the same universe/window.
3. Decision (adopt / don't) recorded with the numbers.

**Estimate:** L. **Dependencies:** E13.3, E7.1.

> **Cross-reference:** the out-of-sample validation E13.3 depends on is the walk-forward task still pending in the backlog (flagged in the "what's missing" discussion). If walk-forward isn't yet a task when E13.3 is picked up, add it first — E13.3's anti-overfitting AC assumes it exists.

---

## EPIC E14 — Stop-Out Diagnosis & Gate/Stop Review (data-driven)

> **Motivation (István, 2026-06-11):** E13.1 showed failures are dominated by STOP-OUTS — price reverses after entry. That has two mutually-exclusive root causes with opposite fixes, and the current data can't yet tell them apart: (A) **stops too tight** — good setups getting wicked out before the move, fixed by widening the stop; (B) **setups lack edge** — price genuinely reverses, fixed by reviewing/adding gates. E14.1 is the diagnostic that decides which branch you're on. Do NOT touch stops or gates until E14.1 says which one. This whole epic is disciplined attribution, not intuition: every change is a measured hypothesis validated on held-out data, one at a time.

### E14.1 — MAE / post-stop excursion diagnostic [measure first — gates everything else]
**Description:** Extend the simulator (E7) and report (E8) to record, for every trade, what happened around and after the stop — the single measurement that distinguishes "stop too tight" from "setup has no edge". No trading-logic change; pure instrumentation, mirroring E13.1's approach.

Per trade, record:
- `mae_r: float` — Maximum Adverse Excursion in R: the furthest price moved AGAINST entry before the eventual exit, ÷ stop distance. (Winners with high MAE = "barely survived"; clustering near −1.0 = stops sit right at the noise boundary.)
- `mfe_r: float` — Maximum Favorable Excursion in R (the symmetric measure; needed to judge whether stopped trades ever offered profit first).
- **Post-stop path** for stop-outs only: continue simulating a *hypothetical* hold for the remaining time-stop window AFTER the stop fires (do NOT change the real trade's outcome — this is a shadow calculation), and record `post_stop_reached_target: bool` and `post_stop_mfe_r: float`. This answers the decisive question: of trades that stopped out, how many would have hit the target within the holding window if the stop hadn't been there?

Report section "Stop-out forensics": of all stop-outs, the % that subsequently reached target (post_stop_reached_target), the distribution of MAE for winners, and the median post-stop MFE. Interpretation lines baked in:
- High % of stop-outs reaching target afterward (say >35%) AND winners' MAE clustering near −1 ⇒ **Branch A, stops too tight** → E14.2.
- Low % reaching target afterward, stopped trades keep falling ⇒ **Branch B, setups lack edge** → E14.3/E14.4.

**Acceptance criteria:**
1. `mae_r`, `mfe_r`, `post_stop_reached_target`, `post_stop_mfe_r` recorded per trade, persisted, queryable.
2. Shadow post-stop simulation provably does not alter the real trade's `exit_reason`/`r_multiple` (golden simulator fixtures from E7.1 unchanged).
3. Hand-built fixtures: a trade that stops then rallies to target ⇒ `post_stop_reached_target=True`; one that stops then keeps falling ⇒ False; each with hand-verified MAE/MFE.
4. Report prints the stop-out-forensics section with the branch interpretation line populated from the actual numbers.

**Estimate:** M. **Dependencies:** E7.1, E8.1, E13.1 (shares the per-trade-attribution plumbing).

### E14.2 — BRANCH A: stop-distance attribution + re-calibration [conditional on E14.1; changes trading logic]
**Description:** Only if E14.1 indicates stops too tight. Add stop distance as a measured dimension (exactly as E13.1 did for targets): bucket trades by stop distance in ATR multiples (0.75, 1.0, 1.5, 2.0, 2.5×) and report n, win rate, expectancy per bucket. Then test wider stop multiples by re-running the backtest with the pullback stop at `EMA20 − k·ATR` for k ∈ {1.0 (current), 1.5, 2.0} and the breakout stop similarly, comparing expectancy. Note the coupling: a wider stop lowers per-trade R for the SAME target, so target distance (in R) must be re-read after — and position sizing (E4.4) auto-adjusts share count since it's risk-based, but verify. Pick the expectancy-maximizing k, validate on held-out data (walk-forward), set per-strategy constants in core.py with the supporting numbers in the task summary.

**Acceptance criteria:**
1. Stop-distance expectancy table rendered per strategy.
2. Backtest re-run at each k with expectancy reported; chosen k traceable to the table in the summary.
3. Chosen k holds on out-of-sample data, not just the tuning window (else revert).
4. Affected golden masters updated with justification.

**Estimate:** M. **Dependencies:** E14.1, walk-forward task.

### E14.3 — BRANCH B step 1: prune dead gates via E8.3 attribution [conditional; changes trading logic]
**Description:** Only if E14.1 indicates setups lack edge. Run the existing E8.3 gate-attribution and act on it: rank current gates by expectancy delta (full-pass expectancy vs failed-only-that-gate). Gates with near-zero delta at n≥30 add no protective value while costing signals — provisionally CUT them (one at a time, re-running the backtest after each to confirm expectancy holds or improves and signal count rises). This is the cheap half of gate review: removing noise before adding anything. Each cut is a trading-logic change with a golden-master update and a recorded before/after.

**Acceptance criteria:**
1. Gate-attribution table produced; each gate labeled keep / cut / insufficient-n (n<30).
2. Each cut gate's removal individually backtested; expectancy and signal-count deltas recorded.
3. Golden masters updated per cut with justification; no gate cut on an n<30 basis.

**Estimate:** M. **Dependencies:** E14.1, E8.3.

### E14.4 — BRANCH B step 2: loser post-mortem → hypothesis-driven new gates [conditional; changes trading logic]
**Description:** Only if E14.1 indicates setups lack edge. The additive half of gate review, run as a disciplined loop, NOT a fishing expedition:
1. **Loser post-mortem:** pull the 20–30 worst stop-outs from the backtest and tabulate shared characteristics at entry — SPY regime (was it BEARISH? — regime is currently display-only, E4.2), distance extended above the 20-MA, RS vs SPY, sector trend, volume quality, ATH zone. Output a short written note of the 2–4 strongest patterns.
2. **For each candidate condition, measure before gating:** add it as a measured dimension (like target/stop distance) and check whether trades satisfying it have materially better expectancy. NO gate yet.
3. **Promote to a real gate only if the data supports it**, one at a time, golden-master update, validated on held-out data. Strong a-priori candidates given the "price reverses" symptom: market regime as a HARD gate (test whether stop-outs cluster in bearish SPY), and extension-from-20MA (buying too far into a move reverts).
4. Stop when expectancy is acceptable or candidates are exhausted — resist stacking many marginal gates (overfitting; each one cuts signal count).

**Acceptance criteria:**
1. Written loser-post-mortem note with the top patterns and their frequency among losers.
2. Each candidate measured as a dimension with an expectancy comparison BEFORE any gate promotion.
3. Each promoted gate: backtested individually, validated out-of-sample, golden master updated, before/after expectancy recorded.
4. No more than one gate promoted per backtest iteration; final gate count and total signal-count cost reported.

**Estimate:** L. **Dependencies:** E14.1, E8.3, E4.2 (regime available as a gateable input).

> **Discipline note (applies to all of E14):** one change per backtest iteration; every change validated on out-of-sample/walk-forward data, not the window it was tuned on; golden masters updated with justification each time. The failure mode to avoid is stacking many marginal stop/gate tweaks that collectively overfit this one backtest — fewer, validated changes beat many plausible ones.

---

## EPIC E15 — Universe Fit & Edge Scoping (why SP600 works, SP400 doesn't)

> **Motivation (István, 2026-06-11):** E14.3 + loser post-mortem found NO single selection gate that predicts individual trade success — accepted as the true shape of the edge (pullback continuation is a positive-expectancy distribution, not a classifier). The live finding that redirects the work: the system is clearly profitable on **SP600 (small-cap)** but significantly weaker on **SP400 (mid-cap)**. The leading hypothesis is a structural cap-class difference — small-caps trend/persist (the pullback resumes), mid-caps mean-revert faster (the "pullback" is more often a genuine reversal), so the strategy's core bet pays in one and not the other. This epic is NOT an attempt to fix SP400 by piling on mid-cap gates (that's the overfitting trap E14 warned against) — it's to (a) confirm the mechanism with the diagnostics already built, and (b) scope the edge to where it lives. Also captures the portfolio-level rule István already added (no new position in a name with a failed trade in the last week), which helped by removing correlated-loser clusters, not by improving selection.

### E15.1 — Capture the "recent-failure cooldown" portfolio rule [changes trading logic — already validated live]
**Description:** Formalize the rule István added to the backtest: suppress a new signal for a ticker that had a stopped-out trade within the last N trading days (default `FAILURE_COOLDOWN_DAYS = 5`). This is a PORTFOLIO/sequencing filter, not a selection gate — it acts at signal-generation time (E6.1) using the journal/trade history, not inside `evaluate`. Mechanism: in the backtest loop and in live scan, before emitting a qualifying signal, check the trade record (E9 DB) for a stop-out on that ticker within the window; if present, suppress (log reason `"failure_cooldown"`, surface as a near-miss-style record so its cost is measurable). Constant in core.py; CLI `--failure-cooldown-days`.

**Acceptance criteria:**
1. Backtest with cooldown on vs off: signal count drops, and the suppressed signals' aggregate expectancy is reported (confirms they were net-negative — the justification for the rule).
2. A ticker stopped out on day D produces no new signal through D+5; a signal on D+6 passes.
3. Lives in the signal loop, NOT in `evaluate` (evaluate stays pure and context-free of portfolio state — verify by grep; `evaluate` must not read the journal).
4. Golden masters for `evaluate` unaffected (this is outside the evaluator).

**Estimate:** M. **Dependencies:** E6.1, E9.3 (trade history available to the loop).

### E15.2 — Side-by-side universe diagnostic (confirm the mechanism) [measurement only]
**Description:** Run the E14.1 stop-out forensics + E13.1 exit-reason/distance attribution on SP400 and SP600 in one comparison report, so the cap-class difference is characterized, not just observed. New report mode `--compare-universes sp600.txt sp400.txt` (or a small driver that runs both and diffs). Surface per universe, side by side: expectancy, win rate, stop-out share, **post-stop-reached-target rate** (the decisive number — higher "keeps falling" on SP400 ⇒ mean-reversion mechanism), winners' MAE distribution, and median holding days. Interpretation block keyed to the result:
- SP400 stop-outs keep falling more than SP600 ⇒ **mean-reversion mechanism** ⇒ scope edge to small-cap (E15.4).
- SP400 noisier across the board (high MAE on winners too) ⇒ **volatility/stop-fit mechanism** ⇒ stop thresholds were small-cap-tuned (could be a per-universe stop calibration, but only if it survives walk-forward).

**Acceptance criteria:**
1. One report rendering both universes' metrics side by side with the deciding post-stop-target-rate prominent.
2. Interpretation line populated from actual numbers, naming the implicated mechanism.
3. No trading-logic change; pure analysis.

**Estimate:** M. **Dependencies:** E14.1, E13.1.

### E15.3 — Trend-strength dimension (the decisive hypothesis test) [measurement only]
**Description:** Test the trend-persistence hypothesis directly. Add a trend-strength measurement at entry — ADX(14) and 200-MA-slope (or distance-from-rising-200MA) — as a measured DIMENSION (not a gate, per E14.4 discipline), and bucket expectancy by it WITHIN each universe. The diagnostic signature: if SP400 is profitable ONLY in its highest trend-strength bucket while SP600 is profitable broadly across buckets, the trend-persistence mechanism is confirmed — mid-caps need a stronger confirmed trend before the pullback bet pays. Output: expectancy-by-trend-strength table per universe, with the verdict line (broad vs concentrated-in-high-trend).

**Acceptance criteria:**
1. Trend-strength buckets with n, win rate, expectancy per universe.
2. Verdict line comparing SP600 (expected: broad) vs SP400 (hypothesis: high-trend-only).
3. Measured as a dimension; no gate promoted in this task.

**Estimate:** M. **Dependencies:** E15.2, E4.2 (200-MA/ADX indicators available).

### E15.4 — Scope the edge to its home turf [decision + config, not new logic]
**Description:** Act on E15.2/E15.3. The DEFAULT recommended action (subject to the data) is to treat universe as a strategy parameter and scope the validated edge to SP600 — i.e. ship SP600 as the production universe, leave SP400 out rather than patching the strategy onto it. Deliverable: a one-page decision note recording the mechanism the diagnostics confirmed, the universes the strategy is and isn't validated on, and the explicit scoping decision. Implementation is config, not code: a documented default universe (`universes/sp600.txt`) and a note in the README that mid-caps are out of scope for THIS strategy and would need separate validation. ONLY if E15.3 shows SP400 works cleanly in a high-trend subset AND that subset survives walk-forward should a mid-cap variant be considered — and then as a separate strategy config with its own golden masters, never as a silent gate bolted onto the small-cap strategy.

**Acceptance criteria:**
1. Decision note exists: mechanism, validated/not-validated universes, scoping decision.
2. Production default universe set to the validated one; README scoping note present.
3. Any mid-cap pursuit explicitly deferred to a separate strategy with its own validation (not merged into the existing one).

**Estimate:** S. **Dependencies:** E15.2, E15.3, walk-forward task.

> **Anti-overfitting reminder (whole epic):** the SP600-works / SP400-doesn't split could be partly regime luck within one window. Everything here is contingent on the SP600 edge surviving WALK-FORWARD across multiple out-of-sample periods. If the home-turf edge itself doesn't hold across time, the universe comparison is moot — validate the edge across time before committing capital. The walk-forward task remains the highest-value missing piece in the backlog.
...
- **Target reliability (E13):** E13.1 (instrumentation) DONE — finding: failures are stop-out-dominated, so target distance is not the leak. E13.2 (conservative target) demoted to low-priority but still worth shipping; E13.3 (ATR target) parked; E13.4 (partial exits) parked with it.
- **Stop-out diagnosis (E14) — done/in progress:** E14.1 done; István went Branch B (E14.3) + post-mortem — finding: no single predictive selection gate exists (accepted as the edge's true shape). Branch A and E14.4 not pursued.
- **Universe fit & edge scoping (E15) — the active thread:** live finding is SP600 works, SP400 doesn't. E15.1 captures the recent-failure cooldown rule (already added live). E15.2 (side-by-side universe diagnostic) + E15.3 (trend-strength dimension) confirm the mechanism; E15.4 scopes the edge to its validated home turf. Default expected outcome: ship SP600, leave SP400 to a separate future strategy. All contingent on walk-forward validation of the SP600 edge.
